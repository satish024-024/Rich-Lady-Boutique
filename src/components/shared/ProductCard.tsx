    ...
    >
      ...
      <div className="w-full aspect-[4/5] bg-card overflow-hidden border border-border-accent/30 rounded-2xl relative shadow-xs group-hover:shadow-luxury group-hover:border-muted-gold/45 transition-all duration-500">
        ...
        {product.collectionTag && (
          <div className="absolute top-4 left-4 bg-forest-green text-primary-bg px-2.5 py-0.5 rounded-full text-[8px] font-sans font-bold uppercase tracking-wider z-10 border border-muted-gold/20 shadow-xs pointer-events-none">
            {product.collectionTag}
          </div>
        )}
      ...
      {/* Info Block */}
      <Link href={`/product/${product.id}`} className="mt-4 flex flex-col items-start px-1 cursor-pointer">
